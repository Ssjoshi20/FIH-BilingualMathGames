...
class EasyCrosswordScreen extends StatefulWidget {
  const EasyCrosswordScreen({Key? key}) : super(key: key);

  @override
  State<EasyCrosswordScreen> createState() => _EasyCrosswordScreenState();
}

class _EasyCrosswordScreenState extends State<EasyCrosswordScreen> {
  ...
  List<List<String>> generateCrossword() {
    List<List<String>> grid = List.generate(5, (_) => List.generate(5, (_) => ""));
    final random = Random();
    for (String answer in selectedAnswers) {
      ...
      while (!placed) {
        ...
        bool horizontal = random.nextBool();  // Mix between horizontal and vertical placement

        if (horizontal) {
          if (col + answer.length <= 5) {
            bool canPlace = true;
