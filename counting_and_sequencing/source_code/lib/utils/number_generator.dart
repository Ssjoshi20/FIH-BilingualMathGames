...
class NumberGenerator {
  ...
  static List<String> generateSequence(int length) {
    ...
    n = random.nextInt(100 - length); // Ensure sequence fits within 0-100 range
    ...
    for (int i = 0; i < length; i++) {
      if (i == missingNumberPosition) {
        ...
        sequence.add(NumberToWord.convert(n + i));
      }
    }
  ...
  static String generateCorrectOption(List<String> sequence) {
    ...
    return NumberToWord.convert(n + missingNumberPosition);
  }
  ...
  static List<String> generateOptions(List<String> sequence) {
    ...
    while (optionsSet.length < 3) {
      ...
      if (option != n + missingNumberPosition) {
        optionsSet.add(NumberToWord.convert(option));
      }
    }

    // Add the correct option
    optionsSet.add(NumberToWord.convert(n + missingNumberPosition));
